...
class SoundSensor:
    def __init__(self, pinNum, callback=None, DEBOUNCE_MS=500):
        self.pinNum = pinNum
        self.DEBOUNCE_MS = DEBOUNCE_MS
        self.user_callback = callback
        self.lastTrigger= utime.ticks_ms()
        try:
            self.pin = machine.Pin(self.pinNum, machine.Pin.IN, machine.Pin.PULL_DOWN)
            self.pin.irq(trigger=machine.Pin.IRQ_RISING, handler=self._irq_handler_wrapper)
